...
```{r}
temp <- read.csv2("data/data_T4s_final.csv", row.names=1) %>% 
  select(1:9) %>% 
...
```{r}
prec <- read.csv2("data/data_T4s_final.csv", row.names=1) %>% 
  select(1:9) %>% 
  mutate(site = gsub("_n", "", site)) %>% 
  merge(read.csv("data/climate_data.csv")) %>% 
...
```{r}
phys <- read.csv2("data/data_T4s_final.csv", row.names=1) %>% 
  select(1:9) %>% 
  mutate(site = gsub("_n", "", site)) %>% 
...
```{r}
chem <- read.csv2("data/data_T4s_final.csv", row.names=1) %>% 
  select(1:9) %>% 
  mutate(site = gsub("_n", "", site)) %>% 
  merge(read.csv("data/soilgrids_data.csv")) %>%
  select(site, plot, T_stock, 
...
```{r}
all <- read.csv2("data/data_T4s_final.csv", row.names=1) %>% 
  select(1:9) %>% 
  mutate(site = gsub("_n", "", site)) %>% 
  merge(read.csv("data/climate_data.csv")) %>% 
  merge(read.csv("data/soilgrids_data.csv")) %>%
