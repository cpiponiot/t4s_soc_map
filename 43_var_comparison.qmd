...
kable(caret_rf_model$results,
  digits = 3,
  caption = "Random Forest Model Performance with Variable Selection"
)
```
...
# Créer le jeu de données d'entraînement (calibration) avec les mêmes
# variables que rf_nosel
train_data <- data %>%
  filter(type == "cal") %>%
  select(!contains("plot"), -site, -type, -lon, -lat, -T_stock, T_stock)
# T_stock réajouté à la fin
...
kable(caret_rf_nosel$results,
  digits = 3,
  caption = "Random Forest Model Performance with All Variable"
)
```
